...
function DocTabs({
  ...
  const [filterbyUser, setfilterbyUser] = useState(1);
  ...
  const fetchDocumentFunc = async (reqObj) => {
    try {
      ...
      const docsData=await getAllDocsData(createdByYou,createdByOthers,userDetailAuth.user.id);
      setDocState(docsData);
    } catch (error) {
      console.error("Error fetching documents:", error);
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (selectedTabIndex == "Total") {
        setDocState(newDocData);
      } else {
        const newdata = newDocData.filter(
  ...
  const docCellTemplate = async (container, options) => {
    ...
    const { name, id, creator_id } = data;
    let createdBy =
      creator_id.id === userDetailAuth.user.id ? "You" : creator_id.full_name;
    const splitName = name.split("_");
    const displayName = splitName.slice(0, -1).join("_");
    try {
      // const recipientData = await fetchRecipientCount(id);
    ...
    try {
      ...
      container.innerHTML = `
        <div class="statusDetails">
          ...
          <div class="subtext" id="pending${id}">${data.pending_count} people haven't completed</div>
        </div>
      `;
    } catch (error) {
      console.error("Error fetching pending recipient count:", error);
  ...
  const expdateCellTemplate = (container, options) => {
    ...
    container.innerHTML = `
      <div class="document-details">
        <div class="document-name">${istDate.getDate()}-${
      istDate.getMonth() + 1
    }-${istDate.getFullYear()} </div>
        <div class="document-status">${remDays} days remaining</div>
      </div>
    `;
  };
  ...
  const modDateCellTemplate = (container, options) => {
    ...
    container.innerHTML = `
      <div class="document-details">
        <div class="document-name">${istDate.getDate()}-${
      istDate.getMonth() + 1
    }-${istDate.getFullYear()} ${hours}:${dt.getMinutes()}:${dt.getSeconds()} ${ampm}</div>
        <div class="document-status">${viewer} has reviewed the document </div>
      </div>
    `;
  };
  ...
  const handleAddedFilterChange = (val) => {
    ...
    if (val == 1) {
      ...
      if(selectedTabIndex=="Total"){
        ...
        newDAta1=newDocData.filter((data)=> data.status== selectedTabIndex)
      }
      setDocState(newDAta1);
      ...
      if(selectedTabIndex=="Total"){
        ...
        newData2=newDocData.filter((data) => data.creator_id.id == userDetailAuth.user.id)
        
      }else{
        newData2 = newDocData.filter(
          (data) => data.creator_id.id == userDetailAuth.user.id && data.status== selectedTabIndex
        );
      }
      
      // console.log("2 : ", newData2);
      setDocState(newData2);

    } else {
      var newData3;
      if(selectedTabIndex=="Total"){
        newData3=newDocData.filter((data) => data.creator_id.id != userDetailAuth.user.id)
      }else{
       newData3 = newDocData.filter(
        (data) => data.creator_id.id != userDetailAuth.user.id && data.status== selectedTabIndex
      );
      }
      setDocState(newData3);
    }
  };
  ...
  const handleRowClick = (e) => {
    ...
    if (e.data.creator_id.id === userDetailAuth.user.id) {
      setshowDel(true);
    } else {
      setshowDel(false);
    }
  ...
  const handleRowDoubleClick = (e) => {
    navigate(`/ViewDocumentPage?docStatus=${e.data.status}`, {
      state: { docData: e.data },
    });
  };
  return (
    <>
      <div className="dashboardData">
        ...
        >
          ...
          <Toolbar>
            <Item location="before">
              <div className="infoText">
                In {itemName}, you have {newDocData.length} documents
              </div>
            </Item>
            <Item name="searchPanel" />
            <Item location="after">
